...
func TestGetWalls(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		var (
			...
			gCan = "gym-can"
			w    = wall.WithHolds{Wall: wall.Wall{Name: "Name"}, Holds: []hold.Hold{hold.Hold{X: 10}}}
			mw   = models.NewWallWithHolds(w)
		)
		defer server.Close()

		service.EXPECT().GetWalls(gomock.Any(), gCan).Return([]*wall.WithHolds{&w}, nil)

		var body struct {
			Data []models.WallWithHolds `json:"data"`
		}
		makeRequest(t, client, stdhttp.MethodGet, []string{server.URL, "gyms", gCan, "walls"}, nil, stdhttp.StatusOK, &body)

		assert.Equal(t, []models.WallWithHolds{mw}, body.Data)
	})
}
...
func TestCreateWall(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		var (
			...
			mw   = models.NewWallWithHolds(wall.WithHolds{Wall: w})
		)
		defer server.Close()

		service.EXPECT().CreateWall(gomock.Any(), gCan, w).Return(&wall.WithHolds{Wall: w}, nil)
...
func TestUpdateWall(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		...
		service.EXPECT().UpdateWall(gomock.Any(), gCan, wCan, w).Return(&wall.WithHolds{Wall: w}, nil)
